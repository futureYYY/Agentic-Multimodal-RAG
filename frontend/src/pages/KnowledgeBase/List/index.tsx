...
const KnowledgeBaseList: React.FC = () => {
  ...
  const loadKnowledgeBases = async () => {
    ...
    try {
      ...
      // 检查是否为 HTML 响应（代理失败）
      if (typeof response === 'string' && (response as string).trim().startsWith('<!DOCTYPE html>')) {
        console.error('API Error: Received HTML response. Proxy might be misconfigured.');
        message.error('API 连接失败，请检查后端服务');
        setKnowledgeBases([]);
        return;
      }
  ...
  const loadModels = async () => {
    try {
      ...
      const adaptedModels = (data as any[]).map(m => ({
        ...m,
        id: m.id,
        name: m.name,
