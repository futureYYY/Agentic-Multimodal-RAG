...
const KnowledgeBaseDetail: React.FC = () => {
  ...
  // 点击解析按钮
  const handleParseClick = async (fileId: string) => {
    const hide = message.loading('正在加载配置...', 0);
    try {
      setCurrentFileId(fileId);
      // 每次打开弹窗时重新加载模型列表，确保是最新的
      await loadModels();
      ...
      children: (
        <div className={styles.tabContent}>
          <Dragger
            ...
            accept=".docx,.pdf,.xlsx,.csv,.txt,.md,.jpg,.jpeg,.png,.bmp,.webp"
            multiple
            disabled={uploading}
          ...
          >
            ...
            <p className="ant-upload-hint">
              支持 Word (.docx)、PDF、Excel (.xlsx)、CSV、TXT、MD、图片 格式
            </p>
          </Dragger>
