...
import { getFileChunks, updateChunk, vectorizeFile, getFileDetail, parseFile, submitChunks, getModelList } from '@/services';
...
const ChunkPreview: React.FC = () => {
  ...
  const handleConfirm = async () => {
    ...
    if (isPreviewMode) {
         try {
          ...
          const vectorModel = currentConfig?.vector_model || currentConfig?.embedding_model;
          await submitChunks(fileId, chunksToSubmit, vectorModel);
  ...
  const handleReconfigure = async () => {
      const hide = message.loading('正在加载配置...', 0);
      try {
        // 强制刷新模型列表，确保数据最新
